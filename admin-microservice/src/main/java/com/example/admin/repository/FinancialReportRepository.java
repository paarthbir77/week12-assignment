...
public interface FinancialReportRepository extends JpaRepository<FinancialReport,Integer>{
	...
	@Query("SELECT new FinancialReport(r.id,r.userID,r.billTotal, r.date, r.month, r.year, r.city) FROM FinancialReport r where r.date = :date AND r.month = :month AND r.year = :year")
	List<FinancialReport> findReportByDate(@Param("date") String date, @Param("month") String month, @Param("year") String year);
	
	@Query("SELECT r.id,r.userID,r.billTotal, r.date, r.month, r.year, r.city FROM FinancialReport r")
